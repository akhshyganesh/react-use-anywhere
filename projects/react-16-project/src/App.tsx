...
const Counter: React.FC = () => {
  ...
  return (
    <div style={{ padding: '20px', fontFamily: 'Arial', border: '2px solid #007bff' }}>
      <h2>React 16.8 Compatibility Test</h2>
      <p style={{ fontSize: '24px', fontWeight: 'bold' }}>Count: {count}</p>
      
      <div style={{ marginBottom: '15px' }}>
        <h3>Standard Buttons (From Service!):</h3>
        <button onClick={incrementCounter}>Increment</button>
        <button onClick={decrementCounter} style={{ marginLeft: '10px' }}>Decrement</button>
        <button onClick={resetCounter} style={{ marginLeft: '10px' }}>Reset</button>
      </div>
      
      <div style={{ marginBottom: '15px' }}>
        <h3>Advanced Functions (From Service!):</h3>
        <button onClick={doubleCounter} style={{ background: '#28a745', color: 'white' }}>
          Double Count
        </button>
        <button 
      ...
      <div style={{ fontSize: '12px', color: '#666', marginTop: '10px' }}>
        💡 All buttons call functions from counterService.ts - a plain TypeScript file!
      </div>
    </div>
  );
};
    ...
    <HookProvider hooks={{ counter: useCounter }}>
      <React.Fragment>
        <div style={{ padding: '20px', maxWidth: '800px', margin: '0 auto' }}>
          <h1>React 16.8 + react-use-anywhere Test</h1>
          <p style={{ fontSize: '16px', color: '#555', marginBottom: '20px' }}>
            This demo shows React hooks being called from a plain TypeScript service file!
            Check <code>src/services/counterService.ts</code>
          </p>
          <Counter />
          <CounterDisplay />
        </div>
